...
func executeTaskBase(scriptPath string, params string) (err error) {
	command := exec.Command(scriptPath, params) // 初始化 command
...
func ExecCommand(classify string, scriptPath string, params string) (err error) {
	...
	} else if classify == "php" {
		logger.Info("start exec php - ", scriptPath)
		err = executeTaskBase(scriptPath, params)
		if err != nil {
			return
		}
	} else {
		err = errors.New("目前仅支持 PHP、Python 和 Shell 类型的脚本")
	}
	return
}
